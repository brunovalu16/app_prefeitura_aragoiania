...
export default function Transporte2({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    if (draft) {
      ...
      setSelectedVehicleId(draft?.provider?.id || null);
      setSelectedTime(draft?.schedule?.selectedTime || null);
    }
  }, [route?.params?.transporteDraft]);
  ...
  async function handleSave() {
    try {
      ...
      // ✅ atualiza a rota anterior (Exameseconsultas) pelo key real dela
      if (returnToKey) {
        navigation.dispatch({
          ...CommonActions.setParams({
            transporteDraft: payload,
            transporteDraftAt: at,
          }),
          source: returnToKey,
        });
      ...
      // fallback (se não veio returnToKey)
      ...
      console.log("❌ handleSave Transporte:", e?.code, e?.message, e);
